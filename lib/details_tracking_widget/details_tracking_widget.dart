  ...
  void getDetails(String orderId) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      setState(() {
        ...
        active = jsonData['status']['active'];
        status = jsonData['status'];

        pending = {
          'state': status['pending']["state"],
          'date': status['pending']["date"]
        };
        assigned = {
          'state': status['assigned']["state"],
          'date': status['assigned']["date"]
        };
        onTheWay = {
          'state': status['on_the_way']["state"],
          'date': status['on_the_way']["date"]
        };
        delivered = {
          'state': status['delivered']["state"],
          'date': status['delivered']["date"]
        };
